    ...
    describe("DASH transaction", () => {
        describe("p2pkh", () => {
            ...
            it("can sign transaction", async () => {
                ...
                const { commandData, rawTx } = SecuxDASH.prepareSign(inputs, { to: outputs[0], utxo: outputs[1] });
                const rsp = await GetDevice().Exchange(commandData);
                signed = SecuxDASH.resolveTransaction(rsp, rawTx, inputs.map(x => x.publickey));
