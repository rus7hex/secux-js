...
export function test(GetDevice) {
    ...
    describe('SecuxBNB.signTransaction()', async () => {
        ...
        it('can create transfer-type transaction', async () => {
            ...
            const pk = SecuxBNB.resolvePublickey(rsp);

            const { commandData, serialized } = SecuxBNB.prepareSign(path, { from, to, amount });
            rsp = await GetDevice().Exchange(commandData);
            signed = SecuxBNB.resolveTransaction(rsp, pk, serialized);
